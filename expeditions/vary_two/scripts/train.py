...
train_dataset = Dataset.from_pandas(
    ...
    labels=train_dataframe[label_name]
)
eval_dataset = Dataset.from_pandas(
    features=eval_dataframe[feature_names],
    labels=eval_dataframe[label_name]
)

reference_train_features = reference_train_features.to(
    float32
)
...
hyperparams = Hyperparams(
    ...
    num_bins=num_bins,
    binned_interval_left=binned_intervals[label_name][0],
    binned_interval_right=binned_intervals[label_name][1],
)
...
eval_sets_labels = to_torch_tensor(
    eval_dataframe[label_name].groupby(
        level="trial_num"
    ).first()
)
...
norm = CenteredNorm(vcenter=0, halfrange=abs(max(binned_intervals[label_name])))
cmap = mpl.colormaps["coolwarm"]
interval = binned_intervals[label_name]
...
ax_dist.set_xticks(plot_ticks[label_name])
ax_expected.set_xticks(plot_ticks[label_name])
ax_expected.set_yticks(plot_ticks[label_name])
...
ax_dist.set_xlabel(r"$\delta C_{" f"{subscript}" r"}$", fontsize=13)
ax_dist.set_ylabel(r"$\log P(\delta C_{" f"{subscript}" r"} \, | \, \textrm{dataset})$", fontsize=13)
ax_expected.set_xlabel(r"Actual $\delta C_{" f"{subscript}" r"}$", fontsize=13)
ax_expected.set_ylabel(r"Predicted $\delta C_{" f"{subscript}" r"}$", fontsize=13)
plt.savefig(
